...
public class Algorithm {
	...
	private static ArrayList<Double> weight_new_key=new ArrayList<Double>(); // weight cá»§a new key
	...
	private static boolean is_GA= true;//náº¿u sá»‘ gen cá»§a quáº§n thá»ƒ hÆ¡n má»™t ná»­a lÃ  cÃ³ fitness =0 thÃ¬ khÃ´ng GA,is_GA=false 
	...
	public void CreateNextGeneration2(Integer num_parent) {
		...
		while(nextGeneration.size()<Algorithm.population_size) {
			...
			for(int i=0;i<num_parent;i++){
				...
				System.out.println("parent "+i+"="+Algorithm.thisGeneration.get(pidx.get(i)).getChromosome()+"\n");
			}
			Genome child;
	...
	public int randomSelection(){
		int gene_i=0;
		gene_i = random.nextInt(Algorithm.fitness_table.size());
		return gene_i;
	}
