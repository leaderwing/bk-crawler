...
public class inputGA {
	...
	public static ArrayList<String> getBestWord() {
		return inputGA.best_word;
	}

	public static ArrayList<Genome> find_bestword_and_create_genome(
		...
		for (int i = 0; i < doc_content.size(); i++) {
			...
			arr_length_of_doc.add(i, arr_content.length);//length of each document to caculate weight of each bestword
		...
		//find keyword has max weight in each document by LNU-LTU
		 ...
		 for (int i = 0; i < doc_content.size(); i++) {
		  ...
		  String best_term_of_doc =""; // tu co trong so lon nhat trong document 
		  // duyet tung document
		  ...
		  for (String key : frequent_term_doc.keySet()) { 
			  ...
			  if (m.find()) { 
				  ...
				  System.out.println("fre term doc size= "+frequent_term_doc.size()+"\n");
		...
		System.out.println("array document contain frequent bestword="
				+ arr_doc_contain_frequent_bestword + "\n");
		...
		for (int i = 0; i < arr_doc_contain_frequent_bestword.size(); i++) {
			...
			System.out.println("weight bestword in document = "
					+ weight_bestword_in_doc + "\n");
