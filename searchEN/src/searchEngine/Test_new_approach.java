...
public class Test_new_approach {
	...
	public static void main(String args[]) {
		...
		while(max_crawl<5000) {
			max_crawl++;
			num_doc_crawled=max_crawl;
			ResultSet rs= Linkqueue.getLink(avg_weight);
			int id=0;
			String link_crawl="";
			try {
				while(rs.next()){
			...
			int length_doc = arr_content.length;
			...
			if ((weight_doc > avg_weight)&& (max_crawl>200)) {
					...
					if (!new_key.get(j).equals("")) {
						...
						if (get) {
							...
							Newkeyword.saveDocument(new_key.get(j),
									link_new_key.get(j), weight,avg_fitness);
