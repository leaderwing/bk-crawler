	...
	//public static void Test() {	
		...
		while(max_crawl<50000) {
			...
			content_doc=content_doc.toLowerCase();
				...
				// má»—i pháº§n tá»­ máº£ng cÃ³ dáº¡ng chuá»—i nhÆ° sau : bÃ³ng_Ä‘Ã¡ lÃ  má»™t mÃ´n thá»ƒ_thao..........
			...
			if ((weight_doc > 3) && (max_crawl > 200)) {
				if (doc_weight.size() == 0) {
					...
					Boolean bool = false;
					//chÃ¨n trá»�ng sá»‘ cá»§a doc vÃ o doc_weight theo chiá»�u giáº£m dáº§n
			...
			if ((doc_weight.size() == num_nst)) {
				...
				for(int j=0;j<new_key.size();j++) {
					if (!new_key.get(j).equals("")) {
						...
						if (get) {
							// kiá»ƒm tra keyword má»›i cÃ³ avg_weight > initial_key
							// hay ko?
		...
		//System.out.println("list link extract ="+list_link_extract);
		 int count_link=0;//Ä‘áº¿m sá»‘ link giá»›i háº¡n dc down
			for(String key: list_link_extract.keySet()) {
				...
				// loáº¡i cÃ¡c link cÃ³ dáº¡ng http://hostname.xxx/abc.png(jpg,exe....)
				 ...
				 Linkcrawled.closeConnect();
  				...
  				if(!is_crawled) {
  					...
  					/*if(weight_doc_current>avg_weight){
  						count_link++;
  						System.out.println("link bá»‹ gioi háº¡n thá»© "+count_link+"\n");
  						if(count_link==10){
  							break;
  						}
  					}*/
