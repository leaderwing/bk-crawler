...
public class VariableState extends ConnectDatabase {
	...
	public static void saveDocument(String keyword, int num_doc_contain_key,
			int num_doc_crawled, int sum_length) {

		connect = ConnectDatabase.connectDb();

		try {
			stmt = connect.createStatement();
			String sql = "INSERT INTO variable_state (keyword,num_doc_contain_key,num_doc_crawled,sum_length) VALUES('"
					+ keyword
					+ "',"
					+ num_doc_contain_key
					+ ","
					+ num_doc_crawled + "," + sum_length + ")";
	...
	public static void update(String keyword, int num_doc_contain_key,
			int num_doc_crawled, int sum_length) {
		...
		try {
			stmt = connect.createStatement();
			String sql = "UPDATE variable_state SET num_doc_contain_key = "+num_doc_contain_key+" ,num_doc_crawled = " +num_doc_crawled+",sum_length ="+sum_length+" WHERE keyword ="+"'"+keyword+"'";
			System.out.println("sql=" + sql);// System.exit(0);
			count_row = stmt.executeUpdate(sql);
